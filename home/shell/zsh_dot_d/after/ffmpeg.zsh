...
ffmpeg-watermark-batch() {
    ...
    touch watermark.txt
    current_dir=$(PWD)
    [ "$(ls *.mp4 | tr " " '\r' | wc -l | awk '{print $1}')" -gt "0" ] && for filename in ./*mp4*; do echo ffmpeg-watermark -t "@bossjones" -f $(python3 -c "import pathlib;p=pathlib.Path('${filename}');print(p)") -c $(python3 -c "import pathlib;p=pathlib.Path('${filename}');print(p)"); done >> watermark.txt
    cat watermark.txt
    bash watermark.txt
